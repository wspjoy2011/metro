...
def check_key(phone_map_keys: dict[str, str], key: str):
    """Check key in PHONE_INTERFACE"""
    if key not in phone_map_keys:
        raise KeyError(f'{key} not in PHONE_INTERFACE')


def encode_to_phone(phone_map_keys: dict[str, str], message: str):
...
def decode_to_phone(phone_map_keys: dict[str, str], codes_line: str):
